...
**顶盖驱动流(lid-driven cavity flow)**是验证流体动力学问题计算方法的一个基准案例。我们可以想象在一个方舱内注入流体，然后在方舱的顶部施加一个水平方向的恒定速度。此时流体会在水平流速的驱动下进行流动直至达到稳定。
...
其中流体的特征速度为$$U$$，舱体尺寸为$$L$$，流体粘度为$$\nu$$。在进行仿真时一般是指定流体的雷诺数、特征速度以及舱体尺寸反算出粘度$$\nu$$进行仿真。而舱体的边界条件为除了顶部边界外其它边界的流体流速均为0。运行`CavityFlow.py`可以得到仿真结果如下。
...
**卡门涡街(Kármán vortex street)**是自然界中常见的一种流体现象，当流体经过钝体时会出现周期性的漩涡脱落。想要模拟卡门涡街最直观的一种方法是求解圆柱绕流问题，我们可以在矩形的流场中放置一个圆柱，当流体经过圆柱后就可能会出现卡门涡街现象。实际上卡门涡街与流体的雷诺数密切相关，当雷诺数比较大时才会出现卡门涡街。对于圆柱绕流问题，雷诺数的计算公式为
...
和顶盖驱动流相比，圆柱绕流的边界条件设置要稍微复杂一些。对于矩形流场的边界，我们需要设置上下边界上的流速为0表示无滑移边界；在左边入口边界上设置流场的来流速度为固定值$$U$$；而在右边出口边界上设置流体速度为左侧对应格子的速度表示出口处流动无扰动(实际上这就是Neumann边界条件)。此外还需要设置圆柱表面上的边界条件，当流体接触到圆柱表面时会速度会变为0。
