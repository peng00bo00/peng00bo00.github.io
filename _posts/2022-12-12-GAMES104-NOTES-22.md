...
和Nanite相关的另一个技术是课程前面提到过的G-buffer和[延迟渲染](/blog/2022/GAMES104-NOTES-07/#deferred-rendering)，我们可以把场景中的各种几何信息记录在G-buffer中从而方便渲染时的计算。
...
[V-buffer](/blog/2022/GAMES104-NOTES-07/#visibility-buffer)是为了提升数据读取效率和缓存利用率而提出的一种技术。V-buffer中不会记录太多的几何信息，一般只保存像素上物体的各种编号。
...
回忆基于[virtual texture](/blog/2022/GAMES104-NOTES-06/#terrain-texture)的技术我们可以为物体不同LoD的纹理烘焙在固定大小的纹理贴图上，在渲染时根据相机的位置和实际需要加载所需的纹理。这种材质表达可以提升缓存利用率以及数据加载效率。
...
因此可以结合前面介绍过的[mesh cluster](#gpu-driven-pipeline-in-assassins-creed)来控制模型的细节。
